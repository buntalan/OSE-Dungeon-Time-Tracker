...
function applyOngoingDamageAdjustment(nodeActor, nodeEffect, rEffectComp)
	...
	if rEffectComp.type == "REGEN" then
		...
		----getRoll(rActor,sType,aDice,nBonus,sName,sAction)
		local rRoll = PowerManager.getRoll(rActor, "heal",aClause.dice,aClause.modifier,rAction.label,"");
		...
		local rRoll = PowerManager.getRoll(rActor, "damage",aClause.dice,aClause.modifier,rAction.label,aClause.dmgtype);
		if EffectManager.isGMEffect(nodeActor, nodeEffect) then
			rRoll.bSecret = true;
		end
