...
function helperHandleSoundDrop(nodeSoundset, _, sRecord)
...
function handleNPCDrop(nodeSoundset, _, sRecord)
	local sType = DB.getValue(nodeSoundset, "type", "");
	if sType ~= "" then
		return;
	end
...
function handleItemDrop(nodeSoundset, _, sRecord)
	local sType = DB.getValue(nodeSoundset, "type", "");
	if sType ~= "" then
		return;
	end
...
function handleStandardSpellDrop(nodeSoundset, _, sRecord)
	local sType = DB.getValue(nodeSoundset, "type", "");
	if sType ~= "" then
		return;
	end
