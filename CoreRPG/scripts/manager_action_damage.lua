...
function encodeActionText(rAction)
	local tOutput = {};
	table.insert(tOutput, Interface.getString("action_damage_tag"));
...
function decodeRollData(rRoll)
	...
	if not rRoll.nOrder then
		local sPattern = string.format("%%[%s.-#(%%d+)", Interface.getString("action_damage_tag"));
		rRoll.nOrder = tonumber(rRoll.sDesc:match(sPattern)) or nil;
	end
	if not rRoll.sRange then
		local sPattern = string.format("%%[%s.-%%((%%w+)%%)%%]", Interface.getString("action_damage_tag"));
		rRoll.sRange = rRoll.sDesc:match(sPattern);
		rRoll.range = rRoll.sRange;
	end
	if not rRoll.sLabel then
		local sPattern = string.format("%%[%s.-%%]([^%%[]+)", Interface.getString("action_damage_tag"));
