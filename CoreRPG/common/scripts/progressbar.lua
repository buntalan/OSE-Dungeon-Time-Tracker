...
--luacheck: globals fillcolor healthbar noautotext reverse source textprefix usagebar
...
function addHandlers()
	if sMaxNodePath then
		DB.addHandler(sMaxNodePath, "onUpdate", self.onMaxChanged);
	end
	if sCurrentNodePath then
		DB.addHandler(sCurrentNodePath, "onUpdate", self.onCurrentChanged);
	end
	if sUsedNodePath then
		DB.addHandler(sUsedNodePath, "onUpdate", self.onUsedChanged);
	end
...
function removeHandlers()
	if sMaxNodePath then
		DB.removeHandler(sMaxNodePath, "onUpdate", self.onMaxChanged);
	end
	if sCurrentNodePath then
		DB.removeHandler(sCurrentNodePath, "onUpdate", self.onCurrentChanged);
	end
	if sUsedNodePath then
		DB.removeHandler(sUsedNodePath, "onUpdate", self.onUsedChanged);
	end
...
function update()
	if bHealthBar then
		sBarFillColor = ColorManager.getHealthColor(1 - self.getPercent(), true);
	elseif bUsageBar then
		sBarFillColor = ColorManager.getUsageColor(1 - self.getPercent(), true);
	end
