...
function len(s)
	...
	while nBytePos <= nByteLen do
		nBytePos = nBytePos + 1;
		while (nBytePos < nByteLen) and StringUTF8Manager.isContinuationByte(s:byte(nBytePos)) do
			nBytePos = nBytePos + 1;
		end
		nLen = nLen + 1;
	end
...
function offset(s, n)
	...
	if n > 0 then
		...
		while (n < 0) and (nBytePos > 0) do
			nBytePos = nBytePos - 1;
			while (nBytePos > 0) and StringUTF8Manager.isContinuationByte(s:byte(nBytePos)) do
				nBytePos = nBytePos - 1;
			end
			n = n + 1;
		end
...
function getSubstringPositive(s, i, j)
	i = StringUTF8Manager.offset(s, i);
	j = StringUTF8Manager.offset(s, j + 1);
	if j then
		j = j - 1;
	end
	return s:sub(i, j);
